...
structure Parser =  struct
  ...
  fun expect_BRACKETS (T_LBRACKET::T_RBRACKET::ts) = SOME ts
    | expect_BRACKETS _ = NONE
                               ...
                               of NONE => NONE
                                | SOME ts => 
                                  (case parse_expr ts
                                    of NONE => NONE
                                     | SOME (e2,ts) => 
                                         SOME (I.ELetFun(s,sym,(makeFunk syms e1),e2),ts))))))))
                                                   ...
                                                   | SOME (s2,ts) =>
                                                       ...
                                                       of NONE => NONE
                                                        | SOME ts => 
                                                            ...
                                                            of NONE => NONE
                                                             | SOME (e3,ts) =>
                                                                 SOME (I.EIf (I.EApp (I.EApp (I.EIdent "equal",e1 ), (I.EVal (I.VList []))) ,e2, I.ELet (s1,I.EApp (I.EIdent "hd",e1), I.ELet (s2,I.EApp (I.EIdent "tl", e1),e3))),ts) 
                                                                 ))))))))))))
   ...
   | SOME ts => 
       ...
       of NONE => NONE
        | SOME (s,ts) => 
            (case parse_expr ts
            of NONE => NONE
             | SOME (recs, ts) => SOME (I.EField (recs,s),ts))))

  and parse_aterm_RECORD ts =
  (case expect_LBRACE ts
  of NONE => NONE
   | SOME ts => 
       (case parseRecordHelper ts
       of NONE => NONE 
        | SOME (recs,ts) => 
            (case expect_RBRACE ts
            of NONE => NONE
             | SOME ts => SOME ((I.ERecord recs), ts))))

  and parseRecordHelper ts =
  (case expect_SYM ts
  of NONE => SOME ([],ts)
   | SOME (s,ts) =>
       (case expect_EQUAL ts 
             ...
             | SOME ts =>
                ...
                of NONE => NONE
                 | SOME (e2,ts) =>
                     ...
                     of NONE => NONE
                      | SOME ts =>
                          SOME (I.EApp (I.EApp (I.EIdent "interval",e1),e2),ts))))))
  and parse_comma_expr ts =
  (case parse_expr ts
  of NONE => SOME ([],ts) 
   | SOME (e,ts) => 
       (case expect_COMMA ts
                                 ...
                                 | SOME ts =>
                                     ...
                                     of NONE => NONE
                                      | SOME (e3,ts) =>
                                          (case expect_RBRACKET ts
                                           of NONE => NONE
                                            | SOME ts => SOME (I.EApp (I.EApp
                                            (I.EIdent "map", I.EFun (s,e1)),
                                            (I.EApp(I.EApp (I.EIdent "filter",
                                            (I.EFun (s,e3))),e2))), ts))))))))))
                       ...
                       | SOME ts =>
                           ...
                           of NONE => NONE
                            | SOME (e2,ts) => 
                                (case expect_RBRACKET ts
                                of NONE => NONE
                                 | SOME ts => SOME (I.EApp (I.EApp (I.EIdent
                                 "map", I.EFun (s,e1)),e2),ts))))))))
    ...
    of NONE => NONE
     | SOME ts => 
         (case parse_comma_expr ts
          ...
          (case expect_RBRACKET ts
              of NONE => NONE
               | SOME ts => SOME ((curriedCons e),ts))))

  and curriedCons eList = List.foldr (fn (x, y) => I.EApp (I.EApp (I.EIdent "cons", x), y)) (I.EVal (I.VList [])) eList

  

  and makeFunk (sym::[]) e1 =
       I.EFun(sym, e1)
    | makeFunk (sym::syms) e1 = 
       I.EFun(sym,(makeFunk syms e1))
    | makeFunk _ _ = parseError "invalid arguments in makeFunc"

  and parse_symlist ts =
    choose [parse_symlist_SYMLIST, parse_symlist_EMPTY] ts 

  and parse_symlist_SYMLIST ts = 
  ...
  and parse_symlist_EMPTY ts = SOME ([], ts)
