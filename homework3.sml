...
fun stringOfExpr e = let
    ...
    fun strE (EVal v) = strCon "EVal" strV [v]
      ...
      | strE (EAccess (eid,e1,e2)) = strCon "EAccess" strE [eid,e1,e2]
in
    strE e
end
...
fun applyEye (VRat (n,d)) = VMat (identityMat n) 
  | applyEye _ = evalError "applyEye"


fun accessRow rowi (row::rows) =
  if rowi = 0 then row
  else accessRow (rowi-1) rows
  | accessRow _ [] = evalError "error row index out of bounds" 

fun accessColumn col (var::vars) =
  if col = 0 then var
  else accessColumn (col-1) vars
  | accessColumn _ [] = evalError "error column index out of bounds"

fun accessIndex ((VRat (row,x1)),(VRat(col,x2))) (VMat mat) = 
  accessColumn col (accessRow row mat)
...
fun subst (EVal v) id e = EVal v
  ...
  | subst (EAccess (f,a,b)) id e = EAccess (subst f id e, subst a id e, subst b id e)
...
fun eval _ (EVal v) = v
  ...
  | eval fenv (EAccess (eid,a,b)) = accessIndex ((eval fenv a),(eval fenv b)) (eval fenv eid)
         ...
         of NONE =>SOME  ([e], ts) 
          | SOME ts => 
              ...
              of NONE => SOME ([e],ts)
               | SOME (es,ts) => (*SOME (e::es,ts))))*)
                   (case parse_expr_row ts
                   of NONE => SOME (e::es,ts)
                    | SOME (ef, ts) => SOME (rev(ef::rev(e::es)),ts)))))
         ...
         of NONE => NONE
          | SOME ts =>
            ...
            of NONE => NONE
             | SOME (exp1,ts) => 
               (case expect_COMMA ts
               of NONE => (case expect_RBRACKET ts 
                          of NONE => NONE
                           | SOME ts => SOME((EAccess ((EIdent id),exp1,(EVal(VRat(0,1))))),ts))
                | SOME ts => 
                  (case parse_expr ts
                  of NONE => NONE
                   | SOME (exp2,ts) => 
                       ...
                       | SOME ts => 
                           ...
                           of NONE => NONE
                            | SOME ts => 
                                ...
                                of NONE => NONE
                                 | SOME (funk,ts) =>
                                     SOME((DeclDefinition(name,args,funk),ts)))))))))
...
fun parse_wdef_wdefshell tokens=
    (case parse_decl tokens
      of SOME (e,[]) => SOME e
       | _ => NONE)
...
fun shell_wdef fenv = let
	...
	(let val ts = lexString str
	     val _ = pr (["Tokens ="] @ (map stringOfToken ts))
	 in
       (case parse_wdef_wdefshell ts
       of NONE => parseError "error evaluating expression" 
        | SOME (DeclDefinition (name,args,expression)) => read ((name,FDef(args,expression))::fenv)
        | SOME (DeclExpression expression) => let
                                             val _ = pr ["Internal rep = ", stringOfExpr expression]
                                             val v = eval fenv expression
                                             val _ = pr [stringOfValue v]
                                             in
                                               read fenv
                                             end)
...
(*added substitution and internal representation for matlab style matrix access
* as well as a poorly named parse_factor_ACCESS which is called in parse_expr
*)
